...
fn url_string(
    ...
    url.push_str("/actions/artifacts/");
    ::querylizer::Simple::extend(&mut url, &p_artifact_id, false, &::querylizer::encode_path)?;
    url.push('/');
    ::querylizer::Simple::extend(&mut url, &p_archive_format, false, &::querylizer::encode_path)?;
    Ok(url)
}
