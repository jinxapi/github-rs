...
fn url_string(
    ...
    url.push_str("/dependency-graph/compare/");
    ::querylizer::Simple::extend(&mut url, &p_basehead, false, &::querylizer::encode_path)?;
    if let Some(value) = &q_name {
        url.push('?');
        ::querylizer::Form::extend(&mut url, "name", value, false, &::querylizer::encode_query)?;
    }
    Ok(url)
}
