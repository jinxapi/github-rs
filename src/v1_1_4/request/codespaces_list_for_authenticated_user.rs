...
fn url_string(
    ...
    q_repository_id: ::std::option::Option<i64>,
    ...
    if let Some(value) = &q_repository_id {
        url.push(::std::mem::replace(&mut prefix, '&'));
        ::querylizer::Form::extend(&mut url, "repository_id", value, false, &::querylizer::encode_query)?;
    }
    Ok(url)
}
