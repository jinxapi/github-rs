...
fn url_string(
    base_url: &str,
    q_featured: ::std::option::Option<bool>,
    ...
    let mut url = String::with_capacity(trimmed.len() + 25);
    url.push_str(trimmed);
    url.push_str("/licenses");
    let mut prefix = '?';
    if let Some(value) = &q_featured {
        url.push(::std::mem::replace(&mut prefix, '&'));
        ::querylizer::Form::extend(&mut url, "featured", value, false, &::querylizer::encode_query)?;
    }
