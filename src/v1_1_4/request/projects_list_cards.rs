...
fn url_string(
    base_url: &str,
    p_column_id: i64,
    q_archived_state: ::std::option::Option<&str>,
    ...
    url.push_str("/cards");
    let mut prefix = '?';
    if let Some(value) = &q_archived_state {
        url.push(::std::mem::replace(&mut prefix, '&'));
        ::querylizer::Form::extend(&mut url, "archived_state", value, false, &::querylizer::encode_query)?;
    }
