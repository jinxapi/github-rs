...
fn url_string(
    ...
    q_head: ::std::option::Option<&str>,
    q_base: ::std::option::Option<&str>,
    ...
    if let Some(value) = &q_head {
        url.push(::std::mem::replace(&mut prefix, '&'));
        ::querylizer::Form::extend(&mut url, "head", value, false, &::querylizer::encode_query)?;
    }
    if let Some(value) = &q_base {
        url.push(::std::mem::replace(&mut prefix, '&'));
        ::querylizer::Form::extend(&mut url, "base", value, false, &::querylizer::encode_query)?;
    }
