...
pub fn http_builder(
    ...
    let default_url = concat!("https://api.github.com", "/user/emails");
    let url = if base_url.is_empty() {
        ::http::uri::Uri::from_static(default_url)
    } else {
        let trimmed = base_url.trim_end_matches('/');
        let mut url = String::with_capacity(trimmed.len() + 12);
...
/// Types for body parameter in [`super::users_delete_email_for_authenticated_user`]
pub mod body {
    #[cfg(feature = "hyper")]
