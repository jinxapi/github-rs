...
fn url_string(
    ...
    p_repo: &str,
    p_tree_sha: &str,
    q_recursive: ::std::option::Option<&str>,
    ...
    ::querylizer::Simple::extend(&mut url, &p_tree_sha, false, &::querylizer::encode_path)?;
    if let Some(value) = &q_recursive {
        url.push('?');
        ::querylizer::Form::extend(&mut url, "recursive", value, false, &::querylizer::encode_query)?;
    }
    Ok(url)
}
